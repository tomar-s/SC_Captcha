...
class ImageSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            random_image_label_new = random_image_label.replace("1", "\\")
            random_image_label_new = random_image_label.replace("2", "/")


            for j, ch in enumerate(random_image_label_new):
                y[j][i, :] = 0
...
def main():
    ...
    # with tf.device('/device:XLA_CPU:0'):
        ...
        log_dir = "/users/pgrad/tomars/shivani_train/logs/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
